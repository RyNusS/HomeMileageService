...
> **상태: v1.28.0 운영 중** — 마일리지 코어(Phase 1), 외부 학습 앱 자동 적립 연동(Phase 2), PC 사용시간 가드(Phase 3), 안드로이드 앱·FCM 푸시, 공지사항·가족 채팅까지 동작한다.
...
| 🔔 실시간 알림 | 부모는 텔레그램 인라인 버튼으로 즉시 승인/거절, 가족 모두 앱 푸시(웹푸시·FCM)로 소식을 받습니다. |
| ✅ 사용권 사용 승인 | 휴대폰·게임기처럼 부모 확인이 필요한 사용권은 사용 신청 → 부모 승인 시 차감됩니다(항목별 on/off). |
| 📢 공지사항·댓글 | 가족 게시판에 글과 사진을 올리고 댓글로 답합니다. 새 글·댓글은 푸시로 알립니다. |
| 💬 가족 채팅 | 메신저형 채팅 탭에서 텍스트·사진을 주고받습니다. 미읽음 배지, 대화 중이 아닐 때만 푸시. |
| 🔌 외부 앱 연동 | 학습 앱 등이 적립 API로 자동 청구(중복 차단 내장). 예: 영어 학습 100점 달성 시 자동 적립. |
| 🖥️ PC 사용시간 가드 | 아이 PC 세션을 잔여 시간만큼만 허용하고, 소진되면 화면을 잠급니다. 실제 사용한 시간만 차감합니다. 부모가 정한 요일별 자유 시간엔 사용권 없이 쓸 수 있고, PC 설정은 부모 화면에서 모든 PC에 한 번에 적용됩니다. 가드는 스스로 업데이트합니다. |
| 📱 웹앱(PWA)·안드로이드 앱 | 홈 화면에 설치해 쓰는 PWA와 같은 화면을 담은 안드로이드 앱(WebView 셸). 앱은 FCM 푸시와 알림음 설정을 지원합니다. |
...
| 역할 | 하는 일 |
|---|---|
| 🧒 **아이** | 아이디+PIN 로그인 · 적립 청구(증빙 사진) · 사용권 구매/사용(필요 시 부모 승인) · 용돈 교환 신청 · 공지·채팅 |
| 🧑‍🍼 **부모** | 청구·사용 승인/거절 · 카탈로그·가격 관리 · 지급/차감 보정 · 용돈 지급 정산 · 가족 내역 · 공지·채팅 |
...
| 알림 | 부모는 텔레그램 봇(인라인 승인 버튼) + 앱 푸시, 자녀는 앱 푸시(웹푸시·FCM). 채팅은 대화 중인 사람에게 푸시 생략 |
| 멀티테넌시 | 가족(테넌트) 단위 데이터 격리 — 처음부터 반영 |
| 클라이언트 | PWA(설치형 웹앱) + 안드로이드 WebView 셸 앱(Capacitor). 화면은 웹 배포만으로 갱신, 앱 재설치는 네이티브 변경 때만 |
| 외부 앱 연동 | 학습 앱 등 외부 클라이언트가 적립 청구 API로 자동 청구(중복 차단 내장) |

## 아키텍처

- **프론트엔드**: React + Vite (PWA) — `web/` · 안드로이드 앱 셸(Capacitor) — `app/`
...
app/      # 안드로이드 앱 셸 (Capacitor WebView, FCM)
pc-guard/ # 자녀 PC 잠금 프로그램 (Python/Tk, Windows exe) — pc-guard/README.md
...
- 사용권 사용 신청(voucher use request): 승인 필요 항목은 `대기 → 승인(차감)/거절`
- 공지(notice)·댓글, 채팅 메시지(chat message): 가족 단위 게시판·대화. 채팅은 사용자별 읽음 위치로 미읽음 수 산출
...
| Phase 3 | 사용권 기기 자동 적용 (PC 세션 가드, 비정상 종료 알림) | ✅ 코어 완료 (v1.9.0) · 요일별 자유 시간·PC 설정 서버 관리·가드 자동 업데이트 (v1.28.0) |
| Phase 4 | 다가족 운영 (관리자 콘솔, 가족 발급) | 일부 (관리자 가족/계정 관리 구현) |
| 소통 기능 | 공지사항·댓글(v1.15~1.17), 가족 채팅(v1.18) | ✅ 완료 |
| 앱 | 안드로이드 셸·FCM 푸시·알림음 설정(v1.6~1.13) | ✅ 완료 · 앱 내 자체 업데이트는 예정 |

## 다음 단계

- 안드로이드 앱 자체 업데이트(앱 안에서 새 버전 설치) → 이후 홈 화면 아이콘 미읽음 배지
