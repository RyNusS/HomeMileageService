#!/usr/bin/env bash
# HMS deploy v1.27.0 (run on server). Bundle at /tmp/hms_deploy.tgz
set -euo pipefail
cd ~/stacks
TS=$(date +%Y%m%d%H%M%S)
...
grep -q '^APP_VERSION=' hms.env && sed -i 's/^APP_VERSION=.*/APP_VERSION=1.27.0/' hms.env || echo "APP_VERSION=1.27.0" >> hms.env
