...
export async function adminRoutes(app) {
  ...
  app.delete('/admin/families/:id', guard, async (req, reply) => {
    ...
    const done = await tx(async (c) => {
      ...
      await c.query('DELETE FROM chat_read WHERE user_id IN (SELECT id FROM app_user WHERE family_id = $1)', [fid]);
      await c.query('DELETE FROM chat_message WHERE family_id = $1', [fid]);
  ...
  app.delete('/admin/users/:id', guard, async (req, reply) => {
    ...
    const done = await tx(async (c) => {
      ...
      await c.query('DELETE FROM chat_read WHERE user_id = $1', [uid]);
      await c.query('DELETE FROM chat_message WHERE user_id = $1', [uid]);
