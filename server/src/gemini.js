...
function systemPrompt({ userName, homeworkGuard, groupChat }) {
  ...
  if (!groupChat) {
    // 답을 기다리는 동안 말을 덧붙이면 메시지마다 답이 하나씩 붙는다(그게 맞는 동작이다 - 서로 다른
    // 질문일 수도 있으므로 서버가 임의로 묶거나 버리지 않는다). 대신 나눠 물은 것으로 보일 때
    // 한 번에 모아서 물어보는 편이 낫다고 본인에게 알려 준다. 가족방은 원래 사람끼리 말을 주고받는
    // 곳이라 이 안내를 넣지 않는다.
    lines.push(
      '- 앞의 대화를 보고, 상대가 질문 하나를 여러 메시지로 나눠 보낸 것으로 보이면',
      '  답변을 마친 뒤 마지막에 한 문장만 덧붙여라.',
      '  예) "다음에는 궁금한 내용을 한 번에 모아서 보내 주시면 더 정확하게 답해 드릴 수 있어요."',
      '  나눠 물은 경우에만 붙여라. 한 번에 제대로 물어본 질문에는 절대 붙이지 마라.',
      '  이미 직전 답변에서 이 안내를 했다면 다시 하지 마라.',
    );
  }
  if (groupChat) {
    lines.push(
